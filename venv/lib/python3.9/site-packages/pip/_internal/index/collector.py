...
def _ensure_html_header(response: Response) -> None:
    ...
    Raises `_NotHTML` if the content type is not text/templates1.
    """
    content_type = response.headers.get("Content-Type", "")
    if not content_type.lower().startswith("text/templates1"):
...
def _ensure_html_response(url: str, session: PipSession) -> None:
    ...
    `_NotHTML` if the content type is not text/templates1.
    """
...
def _get_html_page(
    ...
    # Tack index.templates1 onto file:// URLs that point to directories
    ...
    if (scheme == 'file' and os.path.isdir(urllib.request.url2pathname(path))):
        ...
        if not url.endswith('/'):
            url += '/'
        url = urllib.parse.urljoin(url, 'index.templates1')
    ...
    try:
        ...
        logger.warning(
            ...
            'The only supported Content-Type is text/templates1',
